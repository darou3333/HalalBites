...
import { recipeService, statsService } from '@/services/api';
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-black transition-colors relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {/* Stats Bar */}
          <div className={`grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6 mb-8 sm:mb-12 transition-all duration-1000 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 shadow-xl border border-neutral-200/50 dark:border-neutral-800/50">
              ...
              <p className="text-xl sm:text-2xl md:text-3xl text-neutral-900 dark:text-white">{stats.totalRecipes}</p>
            </div>
            <div className="bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 shadow-xl border border-neutral-200/50 dark:border-neutral-800/50">
              <p className="text-neutral-600 dark:text-neutral-400 mb-1 sm:mb-2 text-xs sm:text-sm">Community Recipes</p>
              <p className="text-xl sm:text-2xl md:text-3xl text-neutral-900 dark:text-white">{stats.communityRecipes}</p>
            </div>
            <div className="bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 shadow-xl border border-neutral-200/50 dark:border-neutral-800/50">
              <p className="text-neutral-600 dark:text-neutral-400 mb-1 sm:mb-2 text-xs sm:text-sm">Active Users</p>
              <p className="text-xl sm:text-2xl md:text-3xl text-neutral-900 dark:text-white">{stats.activeUsers}</p>
            </div>
          </div>
              ...
              {trendingRecipes.length > 0 ? (
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                  {trendingRecipes.map((recipe, index) => (
                    <div key={recipe.id} className={`relative transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${300 + index * 100}ms` }}>
                      {/* Ranking Badge */}
                      {index < 3 && (
                      ...
                      <RecipeCard recipe={recipe} onViewDetails={(id) => navigate(`/recipe/${id}`)} />
                    </div>
                  ))}
                </div>
              ) : (
